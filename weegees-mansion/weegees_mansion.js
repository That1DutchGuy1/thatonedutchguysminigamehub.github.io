...
function buildFloorGeometry(maze) {
  ...
  // Non-window slots: solid wallMat box (collision covered by wallSegs strips).
  ...
  function addOuterSeg(side, index, cx, cz) {
    ...
    // texture (alpha-tested for glass cutouts); all other faces use wallMat
    ...
    const faceMats = [wallMat, wallMat, wallMat, wallMat, wallMat, wallMat];
    let windowFaceIdx;
...
function initThree() {
  ...
  function onSceneTexLoaded() {
    sceneTexLoaded++;
    if (sceneTexLoaded === SCENE_TEX_COUNT) buildFloorScene(0);
  }

  loader.load('wall.png', (tex) => {
    wallTex = tex;
  ...
  loader.load('window-wall.png', (tex) => {
    windowWallTex = tex;
...
(function () {
  ...
  function buildWeatherAudio() {
    ...
    rainAudio = new Audio('rain-thunder.mp3');
    rainAudio.loop = true;
    rainAudio.volume = 1;
    ...
    lightningAudio = new Audio('lightning-flash.mp3');
